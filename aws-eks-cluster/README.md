...
| <a name="provider_aws.us-east-1"></a> [aws.us-east-1](#provider\_aws.us-east-1) | 6.50.0 |
...
| [aws_iam_role_policy.karpenter_list_instance_profiles](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy) | resource |
...
| [kubectl_manifest.karpenter_default_nodepool](https://registry.terraform.io/providers/gavinbunney/kubectl/latest/docs/resources/manifest) | resource |
...
| [kubectl_manifest.karpenter_node_class_capacity_reservation](https://registry.terraform.io/providers/gavinbunney/kubectl/latest/docs/resources/manifest) | resource |
...
| [aws_iam_policy_document.karpenter_list_instance_profiles](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
| [aws_iam_policy_document.karpenter_odcr](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_addons"></a> [addons](#input\_addons) | Map of addon definitions to create. The default Karpenter NodePool (named default) and default EC2NodeClass (named default) are created by Terraform once at cluster creation and then owned by ArgoCD (karpenter-default-nodepool app): Terraform ignores both config changes (ignore\_changes on yaml\_body) and in-cluster changes (ignore\_fields), so karpenter\_nodepool\_spec, karpenter\_declare\_cilium\_startup\_taint (its NodePool half), karpenter\_ami\_selector\_terms, and docker\_storage\_size affect these two objects only at cluster creation — update them via the ArgoCD values afterwards. Do not enable enable\_default\_karpenter\_nodepool on a cluster that already has a NodePool named default: creation is a server-side-apply upsert with force\_conflicts and silently overwrites the existing spec. Do not upgrade to this module version before the cluster is registered with the ArgoCD hub and the karpenter-default-nodepool app is available for it, or the pool is frozen with no updater. Upgrading drains nothing: the previous hash-named NodePool is kept in place (its name is frozen via ignore\_changes on the random\_id keepers) and neutered with limits.cpu=0 — an in-place update that Karpenter's drift hash ignores, so existing nodes are untouched but no new nodes can launch from it; capacity migrates to the new default NodePool through normal expireAfter rotation and consolidation (disruption-budget-limited). Once the legacy pool reports zero NodeClaims, set karpenter\_legacy\_nodepool = false to delete the then-empty pool (deleting an empty pool drains nothing); fresh clusters can set it false at creation to never create the legacy pool. Disabling enable\_default\_karpenter\_nodepool or enable\_default\_karpenter\_nodeclass after ArgoCD has adopted the objects deletes them out from under ArgoCD (selfHeal recreates them); disable the ArgoCD app first. The capacity-reservation EC2NodeClass remains fully Terraform-managed and updates in place. karpenter\_declare\_cilium\_startup\_taint adds node.cilium.io/agent-not-ready (NoSchedule) to the NodePool startupTaints at creation so Karpenter's scheduling simulation waits for the cilium agent instead of double-provisioning; it merges into the effective spec, so it also applies on top of a karpenter\_nodepool\_spec override (overwriting any startupTaints the override defines). It also adds a matching toleration to the coredns, aws-ebs-csi-driver, and aws-mountpoint-s3-csi-driver controller pods via addon configuration\_values, and to the external-secrets Deployments via the chart's global.tolerations (its config defaults wait = true), so a fresh cluster bootstraps before cilium is installed (otherwise bootstrap deadlocks: the taint is only removed by cilium, and cilium arrives after the cluster is registered with ArgoCD); the toleration half applies on every apply, not only at creation. Other optional wait=true releases (kubecost, jupyterhub) are not covered and will deadlock a fresh flag-on cluster if enabled. Requires Kubernetes 1.28+ (older aws-mountpoint-s3-csi-driver addon schemas reject controller.tolerations). Because configuration\_values replaces chart-default tolerations for coredns and the EBS CSI controller, those defaults are replicated inline in cilium\_addon\_configuration\_values; revisit them if an addon schema changes its defaults. Pods that start on a node before the cilium agent is ready run cilium-unmanaged until cilium's unmanaged-pod watcher restarts them (where enabled). Disabling the flag later does not reset addon configuration\_values (an absent value is treated as computed): the inert tolerations remain until an explicit reset. | <pre>object({<br/>    enable_guardduty                    = optional(bool, false)<br/>    enable_aws_load_balancer_controller = optional(bool, true)<br/>    enable_metrics_server               = optional(bool, true)<br/>    enable_cert_manager                 = optional(bool, true)<br/>    enable_aws_for_fluentbit            = optional(bool, false)<br/>    enable_external_dns                 = optional(bool, true)<br/>    enable_karpenter                    = optional(bool, true)<br/>    enable_default_karpenter_nodepool   = optional(bool, true)<br/>    enable_default_karpenter_nodeclass  = optional(bool, true)<br/>    enable_aws_efs_csi_driver           = optional(bool, true)<br/>    enable_argocd                       = optional(bool, false)<br/>    enable_aws_cloudwatch_metrics       = optional(bool, false)<br/>    enable_external_secrets             = optional(bool, false)<br/>    enable_fargate_fluentbit            = optional(bool, false)<br/>    enable_ingress_nginx                = optional(bool, false)<br/>    enable_kube_prometheus_stack        = optional(bool, false)<br/>    enable_crossplane                   = optional(bool, false) // Deprecated, use crossplane module instead<br/>    fluentbit_exclude_paths             = optional(list(string), [])<br/>    aws_for_fluentbit_cw_log_group = optional(any, {<br/>      create          = true<br/>      retention       = 7<br/>      use_name_prefix = false<br/>    })<br/>    external_secrets_config = optional(any, {<br/>      chart_version        = "0.16.2"<br/>      service_account_name = "external-secrets-sa",<br/>      namespace            = "external-secrets",<br/>      wait                 = true<br/>    })<br/>    external_secrets_secrets_manager_arns = optional(list(string), ["arn:aws:secretsmanager:*:*:secret:/argus/*"])<br/>    argocd_config                         = optional(any, {})<br/>    argocd_child_config = optional(object({<br/>      enabled = optional(bool, true)<br/>      root_argo_aws_account_id_to_role = optional(map(string), {<br/>        "533267185808" = "argo_root_core_platform_infra_prod_eks",<br/>        "471112759938" = "argo_root_core_platform_infra_dev_eks",<br/>      })<br/>    }), {})<br/>    karpenter_config = optional(any, {<br/>      chart_version = "1.9.2"<br/>    })<br/>    karpenter_nodepool_spec                = optional(any, null)<br/>    karpenter_declare_cilium_startup_taint = optional(bool, false)<br/>    karpenter_legacy_nodepool              = optional(bool, true)<br/>    karpenter_ami_selector_terms           = optional(list(any), [{ alias = "al2023@latest" }])<br/>    enable_karpenter_capacity_reservation  = optional(bool, false)<br/>    karpenter_capacity_reservation = optional(object({<br/>      ec2_node_class_name = optional(string, "odcr")<br/>      selector_terms      = list(any)<br/>    }), null)<br/>    external_dns_config = optional(object({<br/>      chart_version = optional(string, "1.21.1")<br/>      image_tag     = optional(string, "v0.21.0")<br/>      policy        = optional(string, "upsert-only")<br/>      sources       = optional(list(string), ["service", "ingress", "gateway-httproute"])<br/>    }), {})<br/>    cert_manager_route53_hosted_zone_arns = optional(list(string), ["arn:aws:route53:::hostedzone/*"])<br/>    cert_manager_config = optional(any, {<br/>      chart_version = "v1.20.2"<br/>    })<br/>  })</pre> | `{}` | no |
...
| <a name="input_create_ec2_spot_service_linked_role"></a> [create\_ec2\_spot\_service\_linked\_role](#input\_create\_ec2\_spot\_service\_linked\_role) | Whether to create the account-global AWSServiceRoleForEC2Spot service-linked role. Only one may exist per account, so set this to false on additional clusters that share an account with a cluster that already owns the role. | `bool` | `true` | no |
...
| Name | Description |
| ---- | ----------- |
| <a name="output_argocd_child_role_arn"></a> [argocd\_child\_role\_arn](#output\_argocd\_child\_role\_arn) | ARN of the ArgoCD child service account IAM role |
| <a name="output_argocd_root_role_arn"></a> [argocd\_root\_role\_arn](#output\_argocd\_root\_role\_arn) | ARN of the ArgoCD root service account IAM role |
| <a name="output_argocd_root_role_name"></a> [argocd\_root\_role\_name](#output\_argocd\_root\_role\_name) | Name of the ArgoCD service account IAM role |
...
| <a name="output_external_secrets_role_arn"></a> [external\_secrets\_role\_arn](#output\_external\_secrets\_role\_arn) | ARN of the IAM role used by the External Secrets Operator service account |
