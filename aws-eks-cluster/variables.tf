...
variable "addons" {
  description = "Map of addon definitions to create. karpenter_declare_cilium_startup_taint adds node.cilium.io/agent-not-ready (NoSchedule) to the NodePool startupTaints so Karpenter's scheduling simulation waits for the cilium agent instead of double-provisioning; it merges into the effective spec, so it also applies on top of a karpenter_nodepool_spec override (overwriting any startupTaints the override defines). It also adds a matching toleration to the coredns, aws-ebs-csi-driver, and aws-mountpoint-s3-csi-driver controller pods via addon configuration_values, and to the external-secrets Deployments via the chart's global.tolerations (its config defaults wait = true), so a fresh cluster bootstraps before cilium is installed (otherwise bootstrap deadlocks: the taint is only removed by cilium, and cilium arrives after the cluster is registered with ArgoCD). Other optional wait=true releases (kubecost, jupyterhub) are not covered and will deadlock a fresh flag-on cluster if enabled. Requires Kubernetes 1.28+ (older aws-mountpoint-s3-csi-driver addon schemas reject controller.tolerations). Because configuration_values replaces chart-default tolerations for coredns and the EBS CSI controller, those defaults are replicated inline in cilium_addon_configuration_values; revisit them if an addon schema changes its defaults. Pods that start on a node before the cilium agent is ready run cilium-unmanaged until cilium's unmanaged-pod watcher restarts them (where enabled). Enabling the flag changes the NodePool spec hash and therefore replaces the NodePool: every node on the pool drains in parallel (PDBs honored only until terminationGracePeriod). On clusters that already have the flag enabled, upgrading to this module version updates the three addons in place and rolls their pods, plus the external-secrets pods. Disabling the flag later does not reset addon configuration_values (an absent value is treated as computed): the inert tolerations remain until an explicit reset. Opt in per cluster as a planned change."
  type = object({
