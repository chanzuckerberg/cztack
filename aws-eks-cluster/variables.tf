...
variable "addons" {
  description = "Map of addon definitions to create. The default Karpenter NodePool (named default) and default EC2NodeClass (named default) are created by Terraform once at cluster creation and then owned by ArgoCD (karpenter-default-nodepool app): Terraform ignores both config changes (ignore_changes on yaml_body) and in-cluster changes (ignore_fields), so karpenter_nodepool_spec, karpenter_declare_cilium_startup_taint (its NodePool half), karpenter_ami_selector_terms, and docker_storage_size affect these two objects only at cluster creation — update them via the ArgoCD values afterwards. Do not enable enable_default_karpenter_nodepool on a cluster that already has a NodePool named default: creation is a server-side-apply upsert with force_conflicts and silently overwrites the existing spec. Do not upgrade to this module version before the cluster is registered with the ArgoCD hub and the karpenter-default-nodepool app is available for it, or the pool is frozen with no updater. Upgrading drains nothing: the previous hash-named NodePool is kept in place (its name is frozen via ignore_changes on the random_id keepers) and neutered with limits.cpu=0 — an in-place update that Karpenter's drift hash ignores, so existing nodes are untouched but no new nodes can launch from it; capacity migrates to the new default NodePool through normal expireAfter rotation and consolidation (disruption-budget-limited). Once the legacy pool reports zero NodeClaims, set karpenter_legacy_nodepool = false to delete the then-empty pool (deleting an empty pool drains nothing); fresh clusters can set it false at creation to never create the legacy pool. Disabling enable_default_karpenter_nodepool or enable_default_karpenter_nodeclass after ArgoCD has adopted the objects deletes them out from under ArgoCD (selfHeal recreates them); disable the ArgoCD app first. The capacity-reservation EC2NodeClass remains fully Terraform-managed and updates in place. karpenter_declare_cilium_startup_taint adds node.cilium.io/agent-not-ready (NoSchedule) to the NodePool startupTaints at creation so Karpenter's scheduling simulation waits for the cilium agent instead of double-provisioning; it merges into the effective spec, so it also applies on top of a karpenter_nodepool_spec override (overwriting any startupTaints the override defines). It also adds a matching toleration to the coredns, aws-ebs-csi-driver, and aws-mountpoint-s3-csi-driver controller pods via addon configuration_values, and to the external-secrets Deployments via the chart's global.tolerations (its config defaults wait = true), so a fresh cluster bootstraps before cilium is installed (otherwise bootstrap deadlocks: the taint is only removed by cilium, and cilium arrives after the cluster is registered with ArgoCD); the toleration half applies on every apply, not only at creation. Other optional wait=true releases (kubecost, jupyterhub) are not covered and will deadlock a fresh flag-on cluster if enabled. Requires Kubernetes 1.28+ (older aws-mountpoint-s3-csi-driver addon schemas reject controller.tolerations). Because configuration_values replaces chart-default tolerations for coredns and the EBS CSI controller, those defaults are replicated inline in cilium_addon_configuration_values; revisit them if an addon schema changes its defaults. Pods that start on a node before the cilium agent is ready run cilium-unmanaged until cilium's unmanaged-pod watcher restarts them (where enabled). Disabling the flag later does not reset addon configuration_values (an absent value is treated as computed): the inert tolerations remain until an explicit reset."
  type = object({
    ...
    karpenter_legacy_nodepool              = optional(bool, true)
